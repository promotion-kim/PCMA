...
# If w == "all", run over predefined weights.
...
LAUNCH="accelerate launch --config_file scripts/accelerate_configs/multi_gpu.yaml --num_processes=1 --main_process_port 29500"
...
run_one_weight() {
    ...
    local lm_run_name="${dataset_name}/cpc/lm/(${w})*r_better+(1-${w})*r_safer/gamma_${gamma}"

    echo "============================================================"
    echo "[CPC-MODPO] Running w=${w}, gamma=${gamma}"
    echo "[output_dir] ${output_dir}/${lm_run_name}"
    echo "============================================================"
...
if [[ "${w_arg}" == "all" ]]; then
    for w in "${ALL_W_VALUES[@]}"; do
        run_one_weight "${w}"
    done
else
    run_one_weight "${w_arg}"
fi