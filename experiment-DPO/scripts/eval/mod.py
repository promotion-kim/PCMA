...
for _ in tqdm(range(iters)):
    ...
    for idx in range(prompt_input_ids.shape[0]):
        ...
        records.append({
            "dataset_idx": int(random_indices[idx]),
            "prompt": prompt,
            "mod_output": policy_output_decoded,
            "weight_1": script_args.weight_1,
            "weight_2": script_args.weight_2,
            "f_type": script_args.f_type,
...
jsonl_path = "results{}/outputs/{}records_{}_{}_{}.jsonl".format(
    dataset_prefix,
    output_prefix,
    script_args.weight_1,
    script_args.weight_2,
    script_args.f_type,
)
...
print(f"[saved jsonl records] {jsonl_path}")