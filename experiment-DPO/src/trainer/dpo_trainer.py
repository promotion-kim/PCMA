...
class f_DPOTrainer(DPOTrainer):
    def __init__(self, divergence_type, **kwargs):
    ...
    def dpo_loss(
        ...
        Returns:
            ...
            The chosen_rewards and rejected_rewards tensors contain the rewards for the chosen and rejected responses, respectively.
        """
        x, y = policy_chosen_logps - reference_chosen_logps, policy_rejected_logps - reference_rejected_logps
        if self.divergence_type == "reverse_kl":
            chosen_rewards   = self.beta * x
            rejected_rewards = self.beta * y
        elif self.divergence_type == "forward_kl":
            chosen_rewards   = - self.beta * torch.exp(-x)
            rejected_rewards = - self.beta * torch.exp(-y)
        elif self.divergence_type == "jsd":
            chosen_rewards   = - self.beta * stabilized_log1pexp(-x)
            rejected_rewards = - self.beta * stabilized_log1pexp(-y)
        elif "-divergence" in self.divergence_type:
            parts = self.divergence_type.split('-')
            alpha = float(parts[0]) if parts else None
            assert alpha is not None and alpha > 0
            chosen_rewards   = self.beta / alpha * (-torch.exp(-alpha*x))
            rejected_rewards = self.beta / alpha * (-torch.exp(-alpha*y))
